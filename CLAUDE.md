...
- ~~Auf der Live-Seite kommt ein neu ausgeliefertes Feature auch nach mehrfachem Reload nicht an~~ **Gelöst (09.07.2026).** Root Cause war nicht (nur) eine fehlende Cache-Version-Bump, sondern die Fetch-Strategie selbst: `app/sw.js` beantwortete jede Anfrage cache-first (`caches.match(...) || fetch(...)`) — sobald eine Ressource einmal im Service-Worker-Cache lag, wurde sie für immer aus dem Cache bedient, unabhängig von Reloads. Das eigentliche Update kam nur durch, wenn Safari selbst eine neue Service-Worker-Version erkannte — und genau dieses Erkennen ist in Safari (iOS) unzuverlässig/verzögert, auch im normalen Tab (nicht nur als installierte PWA). Fix: Fetch-Strategie auf **Network-First** umgestellt (immer zuerst Netzwerk versuchen, Response zusätzlich in den Cache schreiben, Cache nur als Fallback bei Offline). Damit zeigt die App bei jedem Start mit Internetverbindung garantiert den aktuellsten Stand, unabhängig vom Service-Worker-Update-Timing. Cache-Version weiterhin bei jeder JS-Änderung hochzählen (jetzt `fitness-v8`) — das bleibt relevant für den Offline-Fallback.
